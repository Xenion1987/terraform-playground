...
function install_openstackclient() {
  cbanner "Proceeding ${FUNCNAME[0]}"
  helper_fix_gcc
  if [[ "${1}" == "latest" ]]; then
    python3 -m pip install --upgrade \
...
function help_install() {
 ...
 SUB-COMMANDS:
  ...
  openstackclient           - Install OpenStack-CLI via pip
...
function main() {
  ...
  install)
    ...
    openstacksdk)
      shift
      install_openstacksdk "${1:-latest}"
      ;;
    openstackclient)
      shift
      install_openstackclient "${1:-latest}"
      ;;
    ...
    terraform)
      shift
      install_terraform "${1:-latest}"
      ;;
    terraformer)
      shift
      install_terraformer "${1:-latest}"
      ;;
    terraform-docs)
      shift
      install_tf_docs "${1:-latest}"
      ;;
    terragrunt)
      shift
      install_terragrunt "${1:-latest}"
      ;;
    tflint)
      shift
      install_tf_lint "${1:-latest}"
      ;;
    trivy)
      shift
      install_trivy "${1:-latest}"
      ;;
    *)
      echo "Missing or wrong SUB-COMMAND - Exit"
      help_install
