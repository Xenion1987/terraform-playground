...
INSTALL_DOCKER_BUILDX="${INSTALLDOCKERBUILDX:-"false"}"
    ...
    if type docker-compose > /dev/null 2>&1; then
        ...
        if [ "${target_compose_arch}" != "x86_64" ]; then
            ...
            if ! type pipx > /dev/null 2>&1; then
                echo "pip installing 'pipx' without '--break-system-packages'..."
                if ! pip3 install --disable-pip-version-check --no-cache-dir --user pipx; then
                    echo "pip installing 'pipx' without '--break-system-packages'... FAILED"
                    echo "pip installing 'pipx' using '--break-system-packages'..."
                    if pip3 install --break-system-packages --disable-pip-version-check --no-cache-dir --user pipx; then
                        echo "pip installing 'pipx' using '--break-system-packages'... OK"
                    else
                        echo "pip installing 'pipx' using '--break-system-packages'... FAILED"
                    fi
                else 
                    echo "pip installing 'pipx' without '--break-system-packages'... OK"
                fi
            ...
            if [ ${exit_code} -ne 0 ]; then
                ...
                echo "pip installing 'docker-compose' without '--break-system-packages'..."
                if ! pip3 install --disable-pip-version-check --no-cache-dir --user "Cython<3.0" pyyaml wheel docker-compose --no-build-isolation; then
                    echo "pip installing 'docker-compose' without '--break-system-packages'... FAILED"
                    echo "pip installing 'docker-compose' using '--break-system-packages'..."
                    if pip3 install --break-system-packages --disable-pip-version-check --no-cache-dir --user "Cython<3.0" pyyaml wheel docker-compose --no-build-isolation; then
                        echo "pip installing 'docker-compose' using '--break-system-packages'... OK"
                    else
                        echo "pip installing 'docker-compose' using '--break-system-packages'... FAILED"
                    fi
                else 
                    echo "pip installing 'docker-compose' without '--break-system-packages'... OK"
                fi
            fi
            rm -rf /tmp/pip-tmp
        else
